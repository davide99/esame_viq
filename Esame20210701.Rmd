...
radioButtons("tipo_grafico", "Tipo grafico", c("Pareto","Treemap","Waffle"))
...
plotOutput("plot", hover=hoverOpts(id="h",delayType="debounce",delay=100))
...
output$plot <- renderPlot({
  ...
  if (tipo_grafico() == 'Pareto'){
    ...
    hovered <- nearPoints(tmp_data, input$h, threshold = 10)
    if(!is.null(hovered) && dim(hovered[1])>0){
      p <- p +
        geom_text(data=hovered, aes(x=Percentuale, label=format_percent(Percentuale)),
                  position = position_dodge(width = .9),
        hjust = 0,
        size = 3)
    }
    ...
    hovered <- nearPoints(waffle_data, input$h, threshold = 10)
    if(!is.null(hovered) && dim(hovered[1])>0){
      p <- p +
        geom_text(data=hovered, aes(x=x, y=y, label=group),
                  position = position_dodge(width = .9),
        hjust = 0,
        size = 3)
    }
