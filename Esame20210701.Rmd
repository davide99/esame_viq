...
### Nota
Nella prima parte dell'esame avevo riportato (in modo non corretto) un problema
riguardo la *data quality*: a causa di una cattiva interpretazione del grafico,
non mi ero reso conto che sia il donut da contorno al grafico, sia la voce
"Other", di fatto, codificavano la stessa informazione complementare. Durante
lo svolgimento della seconda parte, mi sono reso conto che
$100\%-93.5\%=6.5\%=Other$, mentre avevo, precedentemente, considerato che le
percentuali nel grafico fossero riferite al $93.5\%$. Di fatto, questo
...
* A causa della retta delle percentuali cumulate, apprezzare la percentuale dei
  paesi con meno giacimenti risulta difficile
...
### Treemap
Il secondo redesign è la *treemap*. Vantaggi:

* Direct labeling delle nazioni *OPEC* e *non-OPEC*
* Facile individuare le nazioni con i maggiori giacimenti

Svantaggi:

* Confronto tra aree impreciso (legge di Stevens)
* Le etichette *OPEC* e *non-OPEC* potrebbero essere elementi distrattivi
...
### Waffle
Il terzo redesign presentato è un *waffle chart*. Vantaggi:
...
* Nessuna suddivisione tra *OPEC* e *non-OPEC*
...
observeEvent(input$h_pareto,{
  hovered_pareto(nearPoints(pareto_data(), input$h_pareto, threshold=30, yvar="Percentuale", maxpoints=1))
})
...
output$waffle <- renderPlot({
  ...
  if (!is.null(hovered_waffle()) && dim(hovered_waffle())[1]>0){
    ...
    p <- p +
      geom_label(
        data=joined_data,
        aes(x=x, y=y, label=paste0(group, "\n", format_percent(Percentuale))),
        hjust = 0,
        size = 5,
        fill = "white"
      ) 
